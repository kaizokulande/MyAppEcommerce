...
<div class="contain">
        ...
        <div class="content">
            ...
            <div class="products-wrap">
                <div class="products">
                    ...
                    <div class="lot">
                        <lottie-player src="{{asset('../lotties/102600-pink-no-data.json')}}" background="transparent"  speed="0.6"  style="width: 75%;margin:auto" loop autoplay></lottie-player>
                        <span>Pas d'article disponible pour ce categorie.</span>
                    </div>
                    @endif
                    <!-- products -->
                </div>
        ...
        @if ($total_page>1)
                @if ($total_page < 6)
                    @if ($prev_page > 0)
                        <div class="page-surf"><a href="/cat/{{$categorie}}/page/{{$prev_page}}"><button><< </button></a></div>
                    @endif
                    @for ($page = 1;$page<=$total_page;$page++)
                        @if ($current_page == $page)
                        <div class="page-white">{{$page}}</div>
                        @else
                        <div class="page"><a href="/cat/{{$categorie}}/page/{{$page}}"><button>{{$page}}</button></a></div>
                        @endif
                    @endfor
                    @if ($next_page<=$total_page)
                        <div class="page-surf"><a href="/cat/{{$categorie}}/page/{{$next_page}}"><button>>> </button></a></div>
                    @endif
                @else  
                    @if ($current_page < 4)
                        @if ($prev_page > 0)
                        ...
                        @endfor
                        <span> ...</span>
                        <div class="page"><a href="/cat/{{$categorie}}/page/{{$total_page-1}}"><button>{{$total_page-1}}</button></a></div>
                        <div class="page"><a href="/cat/{{$categorie}}/page/{{$total_page}}"><button>{{$total_page}}</button></a></div>
                        <div class="page-surf"><a href="/cat/{{$categorie}}/page/{{$next_page}}"><button>>> </button></a></div>
                    @else
                        <div class="page-surf"><a href="/cat/{{$categorie}}/page/{{$prev_page}}"><button><< </button></a></div>
                        <div class="page"><a href="/cat/{{$categorie}}/page/1"><button>1</button></a></div>
                        <span> ...</span>
                        <div class="page"><a href="/cat/{{$categorie}}/page/{{$current_page-1}}"><button>{{$current_page-1}}</button></a></div>
